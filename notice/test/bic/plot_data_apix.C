...
void plot_data_apix(void)
{
  ...
  //scanf("%d",&CH);  
  // filename here
  sprintf(filename, "apix_2_41_60372.dat");
  TH1F * diff = new TH1F("","",10000,0,10000); 
  // get data file size, size should be less than 2 Gbytes
  fp = fopen(filename, "rb");
  fseek(fp, 0L, SEEK_END);
  file_size = ftell(fp);
  fclose(fp);
  int nevt = file_size/64;
//file_size = 10000;
  ...
  for (int i=0;i<nevt;i++){
    ...
    //itmp = itmp << 24;
    //tcb_trigger_number = tcb_trigger_number + itmp;
    tcb_trigger_fine_time = header[1] & 0xFF;
    
    tcb_trigger_coarse_time = header[2] & 0xFF;
    ltmp = header[3] & 0xFF;
    ltmp = ltmp << 8;
    ...
    diff->Fill(tcb_trigger_time - time_tmp);
    time_tmp = tcb_trigger_time;
