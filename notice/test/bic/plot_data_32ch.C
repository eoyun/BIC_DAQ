...
void plot_data_32ch(void)
{
  ...
  scanf("%d",&CH);  
  // filename here
  sprintf(filename, "bic_daq_42.dat");
...
file_size = 100000;
  ...
  while (data_read < file_size) {
    ...
    if (channel == 0) {
      // get fast data
      for (ch = 1; ch <= 32; ch++) {
        charge[ch - 1] = data[7 * (ch - 1)] & 0xFF;
        itmp = data[7 * (ch - 1) + 1] & 0xFF;
        itmp = itmp << 8;
        charge[ch - 1] = charge[ch - 1] + itmp;
        itmp = data[7 * (ch - 1) + 2] & 0xFF;
        itmp = itmp << 16;
        charge[ch - 1] = charge[ch - 1] + itmp;
        
        timing[ch - 1] = data[7 * (ch - 1) + 3] & 0xFF;
        itmp = data[7 * (ch - 1) + 4] & 0xFF;
        itmp = itmp << 8;
        timing[ch - 1] = timing[ch - 1] + itmp;
        itmp = data[7 * (ch - 1) + 5] & 0xFF;
        itmp = itmp << 16;
        timing[ch - 1] = timing[ch - 1] + itmp;
        
        hit_flag[ch - 1] = data[7 * (ch - 1) + 6] & 0xFF;
    ...
    // for waveform data
      ...
      // get waveform dataplot_data_32ch.C
