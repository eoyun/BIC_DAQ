...
void fast(const int runnum, const int MID, const int CH)
{
  char filename[100];
  FILE *fp;
  int file_size;
    ...
    // for waveform data
      ...
      if (first_waveform) {
        ...
        for (ch = 0; ch < 32; ch++) {
          ...
          wave = new TH2F("", Form("wave_ch%d",CH), wave_length, 0, wave_length * 16,100,-50,50); 
          wave->Reset();
          wave->SetStats(0);
