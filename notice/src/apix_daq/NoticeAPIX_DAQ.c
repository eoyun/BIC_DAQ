...
static void add_device(struct dev_open **list, libusb_device_handle *tobeadded, int sid)
{
  ...
  curr->devh = tobeadded;
  curr->vendor_id = APIX_DAQ_VID;
  curr->product_id = APIX_DAQ_PID;
  curr->serial_id = sid;
  curr->next  = *list;
  *list = curr;
}
...
static int handle_interface_id(struct dev_open **list, int sid, int interface, enum EManipInterface manip_type)
{
  ...
  while (curr) {
    ...
    if (desc.idVendor == APIX_DAQ_VID && desc.idProduct == APIX_DAQ_PID
      ...
      if (manip_type == kInterfaceClaim) {
        if ((ret = libusb_claim_interface(curr->devh, interface)) < 0) {
          fprintf(stdout, "Warning: handle_interface_id: Could not claim interface (%d) on device (%u, %u, %u)\n",
                  interface, APIX_DAQ_VID, APIX_DAQ_PID, sid);
        }
      }
...
libusb_device_handle* nkusb_get_device_handle(int sid) 
{
  ...
  while (curr) {
    if (curr->vendor_id == APIX_DAQ_VID && curr->product_id == APIX_DAQ_PID) {
      if (sid == 0xFF)
        return curr->devh;
...
{
  return USB3Read(sid, 0x20000000);
}

// read APIX data size
int DAQread_DATASIZE(int sid, int ch)
{
  unsigned long addr = 0x30000004;
  addr = addr + ch;
  if( ch <= 0 || ch > 3 ){ return 0;}
  return USB3Read(sid, addr);
}

// read APIX data
void DAQread_DATA(int sid, int ch, int data_size, char *data)
{
  int count;
